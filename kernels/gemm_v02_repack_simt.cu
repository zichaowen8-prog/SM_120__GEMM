...
__global__ void v02_kernel(const half* __restrict__ a, const uint8_t* __restrict__ b,
  ...
  const int nt = n / BN, ni = n % BN;
  const int ktiles = K / BK, groups = K / group_size;
  float acc0 = 0.0f, acc1 = 0.0f;
  const half* ap = a + static_cast<size_t>(m) * K;
  for (int kt = 0; kt < ktiles; ++kt) {
    const uint8_t* bp = b + (static_cast<size_t>(nt) * ktiles + kt) * (BK * BN / 2) + ni / 2;
    #pragma unroll
    for (int ki = 0; ki < BK; ++ki) {
      const int k = kt * BK + ki;
      const uint8_t x = bp[ki * (BN / 2)];
      const int g = k / group_size;
      const half* sp = scales + (static_cast<size_t>(nt) * groups + g) * BN + ni;
...
void launch_v02(const half* a, const uint8_t* b, const half* s, half* c,
                int M, int N, int K, int group, cudaStream_t stream) {
  const size_t pairs = static_cast<size_t>(M) * N / 2;
  v02_kernel<<<(pairs + 255) / 256, 256, 0, stream>>>(a, b, s, c, M, N, K, group);
  CUDA_CHECK(cudaGetLastError());
}

KernelResources query_v02_resources() {
  cudaFuncAttributes a{}; CUDA_CHECK(cudaFuncGetAttributes(&a, v02_kernel));
  int blocks = 0; CUDA_CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&blocks, v02_kernel, 256, 0));
